...
- Confirm issues are enabled on the intended channel repo: `gh api repos/<owner>/<repo> --jq '.has_issues'`. If it returns `false`, stop: the user enables them under repo Settings → General → Features → Issues.
